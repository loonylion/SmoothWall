...
my $prefs_file = "${swroot}/pgraphs/preferences/stored";
###my $crontab = "${swroot}/mods/pgraphs/etc/cron.d/crontab_pgraphs";
my $crontab = "/etc/cron.d/crontab_pgraphs";
...
JVS
	   ...
	   print "<table>";
	   print "<tr><th>/etc/cron.d/ entry</th><th>Collect data in</th><th>Graph name</th><th>Show graph</th><th>Move</th></tr>";
	   ...
	   &get_traceroute;
           print <<STUFF
<br><b>If you are collecting red_avail or ping, please fill in 4 Red side IP addresses or hostnames to ping</b><br>
<table>
<tr><td rowspan=2>
...
e.g. your nearest ISP hop (or 'automatic')<br>
</td><td valign=top>
STUFF
	  ;
...
<table >
<tr>
<td colspan=10><b>If you are collecting temperature, voltage or fan speeds, please select and rename the sensors to be shown</b></td>
</tr>
<tr>
<td>Temperatures</td>
...
ENDX
	...
	print "<br><b>the following output from the 'sensors' command may assist you:</b><pre>";
	open(SENS, "sensors 2>&1 | ");
	while(<SENS>) {
	        my $reply = $_;
		chomp($reply);
		print "$reply<br>";
	...
	close(SENS);	
	print "</pre>\n" ;
	print "<br><B>SMART</B><br><br>Will automatically show the text of any failing attributes if present, but the text of non-failing attributes can also be displayed.<br>\n";
	print "<input id ='smart_show_prefail' type='checkbox' name='smart_show_prefail' ";
	if ($pgraphsset{'smart_show_prefail'} ne "N") {
		print " checked ";
	}
	print ">&nbsp;Show the Pre-fail attributes text at all times<br>";
	print "<input id ='smart_show_oldage' type='checkbox' name='smart_show_oldage' ";
	if ($pgraphsset{'smart_show_oldage'} ne "N") {
		print " checked ";
	}
	print ">&nbsp;Show the Old_age attributes text at all times<br>These can take up to 15 minutes before showing below the graphs.<br><br><br>";
	print <<END
<table class='blank'>
<tr>
...
##		open(ROUT,"/bin/traceroute -n -m 5 -q 2 -w 2 208.67.222.222 2>/dev/null |");
		##3.1 rc5
		open(ROUT,"/usr/bin/traceroute -q 2 208.67.222.222 2>/dev/null |");
		my $n=0;
		while(<ROUT>) {
			$n++;
			chomp;
...
sub get_prefs_from_cgi() {
     if (($cgiparams{'ACTION'} eq "Save and Hide" ) or
	    ...
	    if ($cgiparams{'smart_show_prefail'} eq "on" ) {
	         $pgraphsset{'smart_show_prefail'} = "Y";   
	    } else {
	         $pgraphsset{'smart_show_prefail'} = "N";   
	   }
	    if ($cgiparams{'smart_show_oldage'} eq "on" ) {
	         $pgraphsset{'smart_show_oldage'} = "Y";   
	    } else {
	         $pgraphsset{'smart_show_oldage'} = "N";   
	   }
           foreach $graphname (@graphs) {
              my $dbname = (substr($graphname,0,index($graphname,"-")));
...
sub set_prefs_default() {
	...
	# by default smart just shows attribute text when they fail
	$pgraphsset{'smart_show_prefail'} = "N";   
	$pgraphsset{'smart_show_oldage'} = "N";   
	...
	# but some graphs are off by default and are not collected 
        ...
        $pgraphsset{'smart_collect'} = 'N';
        $pgraphsset{'smart_show'} = 'N';
}
